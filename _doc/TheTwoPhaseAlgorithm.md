...
所有棱块朝向为0并且UD中间层的棱位于中间层|翻转UD层坐标范围(FlipUDSlice)[0, 494 * 2048 - 1]|一阶段;标准解优化
...
FlipUDSlice - UD中间层四个位置的棱块的所有情况+所有棱块朝向 - C(12, 4) * (2 ^ 11) = 495 * 2048 = 1013760
...
> 译注: 对称坐标到底是啥? 对称坐标就是```16 * y + i```, 在数值上, 对称坐标(sym-coordinate)和原始坐标(raw-coordinate)完全相等, 但是它俩表示的意义又不同, 或者, 可以把对称坐标表示为(y, i)一个数对, 而原始坐标只是一个值. 上面可以知道, 对称坐标只会比原始坐标多, 不会比原始坐标少. 对称坐标反倒使规模增加, 那还有什么意义呢? 实际上, 通过上面的ClassIndexToRepresentantArray, 我们知道, 我们不存储所有的对称坐标, 而是只存储代表元.
